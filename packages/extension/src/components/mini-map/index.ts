...
export class MiniMap {
  ...
  private LFCtor: Constructor<LogicFlow>
  ...
  private setViewPort(
    ...
    viewStyle.height = `${
      this.viewPortWidth /
      (this.lf.graphModel.width / this.lf.graphModel.height)
    }px`
  ...
  private drag = (e) => {
    this.dragging = true
    const top = this.viewPortTop + e.y - (this.startPosition?.y ?? 0)
    const left = this.viewPortLeft + e.x - (this.startPosition?.x ?? 0)
    ...
    const centerX =
      (this.viewPortLeft + this.viewPortWidth / 2) / this.viewPortScale
    const centerY =
      (this.viewPortTop + this.viewPortHeight / 2) / this.viewPortScale
    this.lf.focusOn({
      coordinate: {

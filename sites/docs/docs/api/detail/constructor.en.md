...
| themeMode                                | string                                                                    | -        | -          | default support: 'default'、'dark'、'colorful'、'radius'                                                                                                                                                                                                                                                                                       |
...
| overlapMode                              | `OverlapMode`                                                             | -        | -          | Element stacking mode. Supports `STATIC(-1)`, `DEFAULT(0)`, `INCREASE(1)`, `EDGE_TOP(2)`;<br/>Static: -1, stacking does not change on selection and `toFront` has no effect; control order via explicit `zIndex`.<br/>Default: 0, nodes above edges; selected element is temporarily raised and restored when deselected.<br/>Increase: 1, each `toFront` lifts and persists the element’s `zIndex`.<br/>Edge Top: 2, edges always render above nodes (nodes are sorted first then edges); selected element is temporarily raised. |

...
| overlapMode                              | `OverlapMode`                                                             | -        | -          | Element stacking mode. Supports `DEFAULT(0)`, `INCREASE(1)`, and `EDGE_TOP(-1)`. Default is `DEFAULT` (edges at bottom, nodes at top, selected elements at the very top). In `EDGE_TOP` mode, edges always render above nodes; sorting renders nodes first then edges.                                                                                 |
